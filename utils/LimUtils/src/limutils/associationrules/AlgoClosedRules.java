...
import limutils.associationrules.AlgoAgrawalFaster94;
 ...
 * a different type of Itemset object as input. Moreover, the method CALCULATEPROBABILITY() is 
 * different because if an itemset is not closed, we must find its closure to determine
 * its probability. To avoid redundancy as much
...
public class AlgoClosedRules extends AlgoAgrawalFaster94{
	 ...
	 */
	public AssocRules runAlgorithm(Itemsets patterns, String output, double minconf) throws IOException {
		// save the parameters
	 ...
	 * @throws IOException exception if error writing to the output file
	 */
	public AssocRules runAlgorithm(Itemsets patterns, String output, double minconf,
	 ...
	 */
	private AssocRules runAlgorithm(Itemsets patterns, String output)
			throws IOException {
		...
		// 1) When we need to calculate the probability of an itemset (in the method
		// "calculateProbability()") we can use a binary search instead of browsing the whole list.
		...
		for (int k = 2; k < patterns.getLevels().size(); k++) {
			for (Itemset lk : patterns.getLevels().get(k)) {
				...
				for(int item : lk.getItems()) {
					...
					// Now we will calculate the probability and confidence
					// of the rule: itemset_Lk_minus_hm_P_1 ==>  hm_P_1
					double probability = calculateProbability(itemset_Lk_minus_hm_P_1); // THIS COULD BE
																// OPTIMIZED ?
					
					// calculate the confidence of the rule : itemset_Lk_minus_hm_P_1 ==>  hm_P_1
					double conf = lk.getProbability() / probability;
					...
					if(usingLift){
						// if we want to calculate the lift, we need the probability of hm_P_1
						probHm_P_1 = calculateProbability(itemsetHm_P_1);  // if we want to calculate the lift, we need to add this.
						// calculate the lift
						double term1 = lk.getProbability();
						double term2 = probability;
						double term3 = probHm_P_1;
					...
					saveRule(itemset_Lk_minus_hm_P_1, probability, itemsetHm_P_1, probHm_P_1, lk.getProbability(), conf, lift);
	...
	private void apGenrules(int k, int m, Itemset lk, List<int[]> Hm)
		...
		if (k > m + 1) {
			...
			for (int[] hm_P_1 : Hm_plus_1) {
				...
				// We will now calculate the probability of the rule  Lk/(hm_P_1) ==> hm_P_1
				// we need it to calculate the confidence
				double probability = calculateProbability(itemset_Lk_minus_hm_P_1); 
				...
				if(usingLift){
					// if we want to calculate the lift, we need the probability of Hm+1
					probHm_P_1 = calculateProbability(hm_P_1);  
					// calculate the lift of the rule:  Lk/(hm_P_1) ==> hm_P_1
					double term1 = lk.getProbability();
					double term2 = probability;
					
					 lift = term1 / (term2 * probHm_P_1);
				...
				// so we can output it
				saveRule(itemset_Lk_minus_hm_P_1, probability, hm_P_1, probHm_P_1, lk.getProbability(), conf, lift);
	...
	/**
	 * Calculate the probability of an itemset by looking at the frequent patterns
	 * of the same size.
	 ...
	 * @return the probability of the itemset
	 */
	private double calculateProbability(int[] itemset) {
...
//		
        ...
        while( first <= last )
        {
            ...
            if(comparison  > 0 ){
            	...
            	// we have found the itemset, so we return its probability.
            	return patternsSameSize.get(middle).getProbability();
            }
        }
