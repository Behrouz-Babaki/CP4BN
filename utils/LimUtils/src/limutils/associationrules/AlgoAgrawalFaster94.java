		...
		if (k > m + 1) {
			...
			for (int[] hm_P_1 : Hm_plus_1) {
				...
				if(usingLift){
					...
					lift = lk.getProbability() / (probability * probHm_P_1);
...
//        throw new RuntimeException("INVALID SUPPORT - THIS SHOULD NOT HAPPEN BECAUSE ALL ITEMSETS HAVE TO BE FREQUENT");
	 ...
	 */
	protected void saveRule(int[] itemset1, double probItemset1, int[] itemset2, double probItemset2,
			double probability, double conf, double lift) throws IOException {
		ruleCount++;
		...
		if(writer != null){
			...
			rules.addRule(new AssocRule(itemset1, itemset2, probItemset1, probability, conf, lift));
		}
	}
